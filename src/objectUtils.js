export default class ObjectUtils {
  ...
  static isValidChild(child, type, validTypes) {
    ...
    if (child) {
      const childType =
        this.getComponentProp(child, "__TYPE") || (child.type ? child.type.displayName : undefined);
      ...
      try {
        if (process.env.NODE_ENV !== "production" && !isValid) {
          ...
          console.error(
            `Vanilla-Utils: Unexpected type; '${childType}'. Parent component expects a ${messageTypes
              .map((t) => `${t}`)
